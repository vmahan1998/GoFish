...
1.  Fish evaluate local conditions only when `Staging? = true`, `Energy < 75%`, `Ionregulatory stress < 3`, `Thermal stress < 3`, and `SPM < mean SPM.`
...
-   `staging? = true` (i.e., the fish is not actively migrating)

<!-- -->

-   `energy < 75%` (low energy state)

<!-- -->

-   `ionregulatory-stress < 3` (low osmoregulatory stress)

<!-- -->

-   `thermal-stress < 3` (within thermal tolerance)

<!-- -->

-   `SPM < mean SPM` (low turbidity)
...
-   `forage-visits`: Increments by 1 with each foraging event, enabling spatial analysis of foraging intensity.

-   `forager-count`: Tracks the number of agents currently foraging on the patch, reflecting competition density.

-   `forage-species`: Stores a list of species that have foraged on the patch, supporting interspecific competition analysis.
