...
-    **Energetic threshold for foraging**:\
    Fish forage only when energy reserves are low and physiological conditions are stable.\
    *(Reflects field observations that migratory fish do not interrupt migration to feed unless energetically necessary and stress levels are tolerable.)*

-    **Habitat-driven patch choice**:\
    Fish evaluate nearby patches and select the one that best matches their species-specific preferences for salinity, temperature, velocity, depth, and particulate matter.\
    *(Reflects optimal foraging theory and documented environmental preferences in estuarine migratory species.)*

-    **Size- and species-based competition**:\
    Fish will not forage in patches occupied by larger individuals of a different species. They may co-forage with same-species fish or smaller heterospecifics.\
    *(Reflects asymmetric competition commonly observed in estuarine fish assemblages, where dominance is based on both size and species.)*

-   **Fallback foraging**:\
    If the best patch is unavailable due to competition, fish attempt to forage in the next-best nearby patch.\
    *(Reflects adaptive behavior observed in migratory fish when confronted with contested or unsuitable habitat.)*

-    **Crowding reduces foraging success**:\
    Energy gain is divided among all foragers in a patch, simulating diminishing returns in crowded environments.\
    *(Reflects limited prey availability and foraging interference in dense aggregations.)*

These patterns and their corresponding rules are grounded in estuarine fish ecology and support spatially explicit assessment of how migratory fish navigate trade-offs between movement, energy, and competition. They guide both the model logic and its interpretation.
...
-   $N_{fish}$ is the number of foraging agents at that patch (≥ 1 to avoid divide-by-zero)
