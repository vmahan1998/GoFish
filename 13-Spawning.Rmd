...
1.  Female agent initiates spawning if the patch meets habitat criteria.
    -   $S_{patch} <= 0.5$
    -   $d_{patch} <= 3$
    -   $staging? = false$
    -   $STST? = false$
2.  The agent evaluates **neighboring patches** for the **fertilization probability** ($P_{fertilize}$) sexually mature males in the local radius ($n_{males}$)​.
...
2.  Spawning-ready males actively search for the closest spawning-ready female. This search is based on both visual cues (proximity) and pheromone-based cues (chemical signals from the female). Sawning only occurs if all of the following conditions are met:
