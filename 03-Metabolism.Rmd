...
Metabolic energy is then allocated toward multiple internal processes
including swimming, digestion, osmoregulation, and lipid catabolism.
...
| **digestion-penalty** | Penalty applied to digestion when metabolic rate is lower or higher than baseline. |
...
$$
swim-base = Met-base \* motion-ratio
$$

$$
E-base = Met-base \* 0.02
$$

E-creation = Met-base \* 0.05

\$\$

### Metabolic Rate Calculation

At every tick, metabolism is recalculated using:

$$
size-factor = (size / M-max) \^ 0.75
$$

$$
temp-factor = Q10 \^ ((T-env - optimal-temperature) / 10)
$$

$$
age-factor = 1 + (1 - age / max-age) \* 0.5
$$

$$
Metabolism-rate = Met-base \* size-factor \* temp-factor \* age-factor
$$

Metabolic accumulation:

$$
total-metabolism = total-metabolism + M-total
$$

### Efficiency Multipliers

Efficiency is calculated by comparing the current metabolic rate to a
reference baseline:

$$
efficiency = M-total / baseline-metabolism
$$
...
Efficiencies dynamically adjust other submodels in response to
